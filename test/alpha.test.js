...
describe('alpha()', () => {
  describe('setting & getting alpha channel', () => {
    const c = named('red');
    test('no arguments gets alpha', () => expect(alpha(c)).toStrictEqual(1));
    test('setting alpha to 0.5', () => expect(alpha(alpha(c, 0.5))).toStrictEqual(0.5));
    test('alpha is unchanged', () => expect(alpha(c)).toStrictEqual(1));
  });
  describe('interpolating alpha channel', () => {
    const c1 = alpha(named('white'), 0);
    const c2 = alpha(named('black'), 1);
    const c = mix(c1, c2, 0.3, 'rgb');
    test('hex is #b3b3b3', () => expect(toHex(c, 'rgb')).toStrictEqual('#b3b3b3'));
    test('hex with alpha', () => expect(toHex(c)).toStrictEqual('#b3b3b34d'));
    test('alpha is 30%', () => expect(alpha(c)).toStrictEqual(0.3));
  });
  describe('constructing rgba color, hsl shorthand', () => {
    const c = alpha(rgb(255, 0, 0, 0.5), 0.5);
    test('color is red', () => expect(toName(c)).toStrictEqual('red'));
    test('alpha is 50%', () => expect(alpha(c)).toStrictEqual(0.5));
  });
  describe('parsing hex rgba colors', () => {
    const c = hex('#ff00004d');
    test('color is red', () => expect(toName(c)).toStrictEqual('red'));
    test('alpha is 30%', () => expect(alpha(c)).toStrictEqual(0.3));
    test('rgba output', () => expect(toRgba(c)).toStrictEqual([255, 0, 0, 0.3]));
  });
  describe('parsing css rgba colors', () => {
    const c = css('rgba(255 255 0 .3)');
    test('color is yellow', () => expect(toName(c)).toStrictEqual('yellow'));
    test('alpha is 30%', () => expect(alpha(c)).toStrictEqual(0.3));
    test('rgba output', () => expect(toRgba(c)).toStrictEqual([255, 255, 0, 0.3]));
  });
  describe('parsing css rgba colors (percentage)', () => {
    const c = css('rgba(100% 0% 0% 0.2)');
    test('color is red', () => expect(toName(c)).toStrictEqual('red'));
    test('alpha is 30%', () => expect(alpha(c)).toStrictEqual(0.2));
    test('rgba output', () => expect(toRgb(c)).toStrictEqual([255, 0, 0]));
    test('rgba output', () => expect(toRgba(c)).toStrictEqual([255, 0, 0, 0.2]));
  });
  describe('parsing hsla colors', () => {
    const c = css('hsla(0 100% 50% 0.25)');
    test('color is red', () => expect(toName(c)).toStrictEqual('red'));
    test('alpha is 25%', () => expect(alpha(c)).toStrictEqual(0.25));
    test('rgba output', () => expect(toRgb(c)).toStrictEqual([255, 0, 0]));
    test('rgba output', () => expect(toRgba(c)).toStrictEqual([255, 0, 0, 0.25]));
  });
  ...
  describe('gl output', () => {
    ...
    test('color is red', () => expect(toName(c)).toStrictEqual('red'));
    test('alpha is 25%', () => expect(alpha(c)).toStrictEqual(0.25));
    test('gl output', () => expect(toGl(c)).toStrictEqual([1, 0, 0, 0.25]));
  });
  describe('rgba css output', () => {
    const c = css('hsla(0 100% 50% 0.25)');
    test('color is red', () => expect(toName(c)).toStrictEqual('red'));
    test('alpha is 25%', () => expect(alpha(c)).toStrictEqual(0.25));
    test('css output', () => expect(toCss(c)).toStrictEqual('rgba(255 0 0 0.25)'));
  });
  describe('hex output', () => {
    const c = gl(1, 0, 0, 0.25);
    test('hex', () => expect(toHex(c)).toStrictEqual('#ff000040'));
    test('rgb', () => expect(toHex(c, 'rgb')).toStrictEqual('#ff0000'));
    test('rgba', () => expect(toHex(c, 'rgba')).toStrictEqual('#ff000040'));
    test('argb', () => expect(toHex(c, 'argb')).toStrictEqual('#40ff0000'));
  });
  describe('num output', () => {
    const c = gl(1, 0, 0, 0.25);
    test('num ignores alpha', () => expect(toNum(c)).toStrictEqual(0xff0000));
  });
  describe('setting alpha returns new instance', () => {
    ...
    test('alpha stays the same', () => {
      expect(c).not.toBe(d);
      expect(alpha(c)).toStrictEqual(1);
      expect(alpha(d)).toStrictEqual(0.5);
    });
  });
});
