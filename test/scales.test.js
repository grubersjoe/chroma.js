...
describe('scale()', () => {
  ...
  test('simple rgb scale (white-->black)', () => {
    ...
    expect(toHex(s(0))).toStrictEqual('#ffffff');
    expect(toHex(s(0.5))).toStrictEqual('#808080');
    expect(toHex(s(1))).toStrictEqual('#000000');
  });
  ...
  test('simple hsv scale (white-->black)', () => {
    ...
    expect(toHex(s(0.5))).toStrictEqual('#808080');
    expect(toHex(s(1))).toStrictEqual('#000000');
    expect(toHex(s.colors())).toStrictEqual(['#ffffff', '#000000']);
    expect(toHex(s.colors(2))).toStrictEqual(['#ffffff', '#000000']);
  ...
  test('simple hsv scale (white-->black), classified', () => {
    ...
    expect(toHex(s(0.5))).toStrictEqual('#808080');
    expect(toHex(s(1))).toStrictEqual('#000000');
    expect(toHex(s.colors(7))).toStrictEqual([
      '#ffffff',
      '#d5d5d5',
      '#aaaaaa',
      '#808080',
  ...
  test('simple lab scale (white-->black)', () => {
    ...
    expect(toHex(s(0))).toStrictEqual('#ffffff');
    expect(toHex(s(0.5))).toStrictEqual('#777777');
    expect(toHex(s(1))).toStrictEqual('#000000');
  });

  test('colorbrewer scale', () => {
    const s = scale(brewer('RdYlGn'));
    expect(toHex(s(0))).toStrictEqual('#a50026');
    expect(toHex(s(0.5))).toStrictEqual('#ffffbf');
    expect(toHex(s(1))).toStrictEqual('#006837');
  });

  test('colorbrewer scale domained', () => {
    const s = scale(brewer('RdYlGn')).domain([0, 100]);
    expect(toHex(s(0))).toStrictEqual('#a50026');
    expect(toHex(s(10))).not.toStrictEqual('#ffffbf');
    expect(toHex(s(50))).toStrictEqual('#ffffbf');
    expect(toHex(s(100))).toStrictEqual('#006837');
  });
  ...
  test('colorbrewer scale - domained - classified', () => {
    ...
    expect(toHex(s(50))).toStrictEqual('#ffffbf');
    expect(toHex(s(100))).toStrictEqual('#006837');
    expect(toHex(s.colors(5))).toStrictEqual([
      '#a50026',
      '#f98e52',
      '#ffffbf',
      '#86cb67',
  ...
  test('calling domain with not arguments', () => {
    ...
    expect(s.classes()).toStrictEqual([0, 20, 40, 60, 80, 100]);
  });
  ...
  test('simple num scale (white-->black)', () => {
    ...
    expect(toHex(s(0))).toStrictEqual('#ffffff');
    expect(toHex(s(0.25))).toStrictEqual('#bfffff');
    expect(toHex(s(0.5))).toStrictEqual('#7fffff');
    expect(toHex(s(0.75))).toStrictEqual('#3fffff');
    expect(toHex(s(0.95))).toStrictEqual('#0ccccc');
    expect(toHex(s(1))).toStrictEqual('#000000');
  });

  test('css rgb colors have rounded rgb() values', () => {
    const c = toCss(scale(brewer('YlGnBu'))(0.3));
    expect(c).toStrictEqual('rgb(170 222 183)');
  });

  test('css rgba colors dont round alpha value', () => {
    const s = scale(brewer('YlGnBu'))(0.3);
    const c = toCss(alpha(s, 0.675));
    expect(c).toStrictEqual('rgba(170 222 183 0.675)');
  });

  test('get colors from scale', () => {
    const s = scale(['yellow', 'darkgreen'].map(named));
    expect(toHex(s.colors())).toStrictEqual(['#ffff00', '#006400']);
    expect(toHex(s.colors(5))).toStrictEqual([
      '#ffff00',
      '#bfd800',
      '#80b200',
    ...
    expect(toCss(s.colors(3, 'css'))).toStrictEqual([
      'rgb(255 255 0)',
      'rgb(128 178 0)',
      'rgb(0 100 0)',
    ]);
  ...
  test('get colors from a scale with more than two colors', () => {
    ...
    expect(toHex(s.colors())).toStrictEqual(['#ffff00', '#ffa500', '#006400']);
  });
  ...
  test('scale padding, simple', () => {
    ...
    expect(toHex(s(0))).toStrictEqual('#e64f35');
    expect(toHex(s(0.5))).toStrictEqual('#ffffbf');
    expect(toHex(s(1))).toStrictEqual('#5d91c3');
  });

  test('scale padding, one-sided', () => {
    const s = scale(brewer('OrRd')).padding([0.2, 0]);
    expect(toHex(s(0))).toStrictEqual('#fddcaf');
    expect(toHex(s(0.5))).toStrictEqual('#f26d4b');
    expect(toHex(s(1))).toStrictEqual('#7f0000');
  });

  test('colors return original colors', () => {
    const s = scale(['red', 'white', 'blue'].map(named));
    expect(toHex(s.colors())).toStrictEqual(['#ff0000', '#ffffff', '#0000ff']);
  });

  test('scale with one color', () => {
    const s = scale(named('red'));
    expect(toHex(s(0.3))).toStrictEqual('#ff0000');
  });

  test('scale() no data color', () => {
    const s = scale(brewer('OrRd'));
    expect(toHex(s(null))).toStrictEqual('#cccccc');
    expect(toHex(s(undefined))).toStrictEqual('#cccccc');
    expect(toHex(s.nodata(hex('#eee'))(null))).toStrictEqual('#eeeeee');
  });

  test('scale wrapped in a scale', () => {
    const s1 = scale(brewer('OrRd'));
    const s = scale(brewer('OrRd')).domain([0, 0.25, 1]);
    expect(s(0)).toStrictEqual(s1(0));
    expect(s(1)).toStrictEqual(s1(1));
    expect(s(0.125)).toStrictEqual(s1(0.25));
    expect(s(0.25)).toStrictEqual(s1(0.5));
    expect(s(0.625)).toStrictEqual(s1(0.75));
  });
});
